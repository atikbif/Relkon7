...
void konlite_call(void)
{
	...
	ip_buf = get_buf_ip_pkt();
	if(ip_buf) {
		send_ip_without_id_increment(ip_buf);
		clear_buf_ip_pkt();
	}else {
					...
					switch(get_eth_type(m_pkt))
					{
						...
						case ARP_TYPE:
							...
							if(check_arp_req(arp_p))	// arp запрос
							{
								...
								arp_answer(arp_p);
							}else if(check_arp_answer_gate(arp_p))	// arp ответ от шлюза
							{
								set_gate_mac(arp_p->mac_s);
							}else if(check_arp_answer(arp_p))	// arp ответ
							{
								add_arp_tab(arp_p->ip_s,arp_p->mac_s);
							}
							break;
		...
		k_tmr++;
		if(k_tmr == GATE_PER) {k_tmr=0;get_gate_mac();}	// периодический запрос mac шлюза
	}
