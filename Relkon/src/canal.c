...
extern volatile unsigned char _Sys_SPI_Buzy;	// флаг занятости SPI другим процессом

// массив временных задержек для RS485 на разных скоростях
static const unsigned char _Sys_Delay[9]={45,20,10,6,4,2,1,1,1};

// приёмные и передающие буферы
...
unsigned char* get_can_rx_ptr(unsigned char num)	// возвращает указатель на приёмный буфер канала
{
	unsigned char *ptr=0;
	switch(num)
	{
...
void clear_rx_cnt(unsigned char num)	// сбрасывает счётчик принятых байт
{
	switch(num)
	{
...
unsigned short get_rx_cnt(unsigned char num)	// возвращает счётчик принятых байт
{
	switch(num)
	{
...
void PultCanTask( void *pvParameters )
{
	...
	for( ;; )
	{
		// проверка наличия байт в буфере и таймаута тишины
...
void BinCanTask( void *pvParameters )
{
	unsigned short tmp;
	init_pc_canal();
	pc_message_bin();	// сообщение приветствия
	...
	for( ;; )
	{
		// проверка наличия байт в буфере, таймаута тишины и флага разрешения протокола
